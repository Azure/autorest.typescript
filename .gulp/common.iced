...
module.exports =
  ...
  autorest: (args,done,ignoreexitcode) ->
    ...
    execute "#{basefolder}/node_modules/.bin/autorest \"--clear-output-folder\" \"--verbose\" \"--debug\" \"--use=#{basefolder}\" #{args.map((a) -> "\"#{a}\"").join(' ')}" , {silent:true, ignoreexitcode: ignoreexitcode || false}, (code,stdout,stderr) ->
