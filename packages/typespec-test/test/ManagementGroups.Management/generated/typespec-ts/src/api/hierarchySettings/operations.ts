...
import {
  ...
  _hierarchySettingsListDeserializer,
...
export function _updateSend(
  ...
  return context
    ...
    .patch({
      ...
      body: createOrUpdateSettingsRequestSerializer(
...
export async function _listDeserialize(
  result: PathUncheckedResponse,
): Promise<_HierarchySettingsList> {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    const error = createRestError(result);
    error.details = errorResponseDeserializer(result.body);
  ...
  return _hierarchySettingsListDeserializer(result.body);
}
...
export function list(
  ...
  groupId: string,
  options: HierarchySettingsListOptionalParams = { requestOptions: {} },
): PagedAsyncIterableIterator<HierarchySettingsInfo> {
  return buildPagedAsyncIterator(
    context,
    () => _listSend(context, groupId, options),
    _listDeserialize,
    ["200"],
    { itemName: "value" },
  );
