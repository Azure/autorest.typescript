...
import {
  ...
  azureSearchDocumentsKnowledgeBaseKnowledgeBaseRetrievalRequestSerializer,
  AzureSearchDocumentsKnowledgeBaseKnowledgeBaseRetrievalResponse,
  azureSearchDocumentsKnowledgeBaseKnowledgeBaseRetrievalResponseDeserializer,
...
export function _retrieveSend(
  ...
  retrievalRequest: AzureSearchDocumentsKnowledgeBaseKnowledgeBaseRetrievalRequest,
  ...
  return context
    ...
    .post({
      ...
      body: azureSearchDocumentsKnowledgeBaseKnowledgeBaseRetrievalRequestSerializer(
        retrievalRequest,
      ),
    });
}
...
): Promise<AzureSearchDocumentsKnowledgeBaseKnowledgeBaseRetrievalResponse> {
  const expectedStatuses = ["200", "206"];
  if (!expectedStatuses.includes(result.status)) {
    throw createRestError(result);
  }

  return azureSearchDocumentsKnowledgeBaseKnowledgeBaseRetrievalResponseDeserializer(
    result.body,
  );
}
