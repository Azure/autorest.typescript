...
export async function _refreshCountersDeserialize(
  result: PathUncheckedResponse,
): Promise<void> {
...
export async function _getCountersDeserialize(
  result: PathUncheckedResponse,
): Promise<RuleCounter> {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
...
export function $delete(
  ...
  return getLongRunningPoller(
    ...
    ["200", "202", "204", "201"],
    {
      ...
      getInitialResponse: () =>
        _$deleteSend(context, globalRulestackName, priority, options),
