...
export async function changeVnet(
  context: Client,
  resourceGroupName: string,
  name: string,
  vnetInfo: VirtualNetworkProfile,
  ...
  const result = await _changeVnetSend(context, resourceGroupName, name, vnetInfo, options);
  return _changeVnetDeserialize(result);
}
