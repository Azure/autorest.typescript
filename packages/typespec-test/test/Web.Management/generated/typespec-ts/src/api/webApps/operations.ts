...
export function listProcessModulesSlot(
  ...
  return buildPagedAsyncIterator(
    context,
    () => _listProcessModulesSlotSend(context, resourceGroupName, name, slot, processId, options),
...
export function listProcessThreadsSlot(
  ...
  return buildPagedAsyncIterator(
    context,
    () => _listProcessThreadsSlotSend(context, resourceGroupName, name, slot, processId, options),
...
export async function _getProcessDumpSlotDeserialize(
  ...
  return __PLACEHOLDER_o1724_sdeserializer__(result.body);
}
...
export async function getProcessDumpSlot(
  ...
  const result = await _getProcessDumpSlotSend(
    context,
    resourceGroupName,
    name,
    slot,
...
export async function getInstanceProcessDumpSlot(
  ...
  const result = await _getInstanceProcessDumpSlotSend(
    context,
    resourceGroupName,
    name,
    slot,
...
export async function getInstanceProcessDump(
  ...
  const result = await _getInstanceProcessDumpSend(
    context,
    resourceGroupName,
    name,
    instanceId,
...
export async function _createFunctionDeserialize(result: PathUncheckedResponse): Promise<void> {
  const expectedStatuses = ["201", "200", "202"];
  if (!expectedStatuses.includes(result.status)) {
    const error = createRestError(result);
    error.details = defaultErrorResponseDeserializer(result.body);
...
export async function getDeploymentSlot(
  ...
  const result = await _getDeploymentSlotSend(context, resourceGroupName, name, slot, id, options);
...
export function restoreSlot(
  ...
  return getLongRunningPoller(context, _restoreSlotDeserialize, ["202", "200", "201"], {
    ...
    abortSignal: options?.abortSignal,
    getInitialResponse: () =>
      _restoreSlotSend(context, resourceGroupName, name, slot, backupId, request, options),
    resourceLocationConfig: "location",
  }) as PollerLike<OperationState<void>, void>;
}
