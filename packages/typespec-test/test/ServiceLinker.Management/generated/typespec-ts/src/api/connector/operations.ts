...
export function validate(
  ...
  ValidateOperationResult
> {
  return getLongRunningPoller(context, _validateDeserialize, ["202", "200"], {
...
export async function _listDryrunDeserialize(
  result: PathUncheckedResponse,
): Promise<_DryrunList> {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
...
export async function _deleteDryrunDeserialize(
  result: PathUncheckedResponse,
): Promise<void> {
...
export async function deleteDryrun(
  ...
  const result = await _deleteDryrunSend(
    context,
    resourceGroupName,
    location,
    dryrunName,
...
export async function getDryrun(
  ...
  const result = await _getDryrunSend(
    context,
    resourceGroupName,
    location,
    dryrunName,
