...
export function _getCertificateSend(
  ...
  const path = expandUrlTemplate(
    ...
    {
      "certificate-name": certificateName,
      "certificate-version": options["certificateVersion"],
      "api%2Dversion": context.apiVersion,
    },
    {
      allowReserved: options?.requestOptions?.skipUrlEncoding,
...
export function _updateCertificateSend(
  ...
  return context
    ...
    .patch({
      ...
      body: {
        policy: !options?.certificatePolicy
          ? options?.certificatePolicy
          : certificatePolicySerializer(options?.certificatePolicy),
        attributes: !options?.certificateAttributes
          ? options?.certificateAttributes
          : certificateAttributesSerializer(options?.certificateAttributes),
        tags: options?.tags,
      },
    });
}
