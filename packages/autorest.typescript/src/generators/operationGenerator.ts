...
function writeLroOperationBody(
  ...
  const commonOptions = `intervalInMs: options?.updateIntervalInMs${lroResourceLocationConfig
    ? `, ${useLegacyLro ? "lroResourceLocationConfig" : "resourceLocationConfig"}: "${lroResourceLocationConfig.toLowerCase()}"`
    : ""
    }`;
  methodDeclaration.addStatements([
    ...
    `const poller = ${useLegacyLro ? `new LroEngine(lro, { resumeFrom: options?.resumeFrom, ${commonOptions} })` : `await createHttpPoller<${responseName}, OperationState<${responseName}>>(lro, { restoreFrom: options?.resumeFrom, ${commonOptions} })`};`,
    "await poller.poll();",
    "return poller;"
  ]);
