...
export interface RemovedContextOptions extends ClientOptions {}
...
export default function createClient(
  ...
  version: Versions,
  options: RemovedContextOptions = {},
): RemovedContext {
  const endpointUrl =
    options.endpoint ??
    options.baseUrl ??
    `${endpointParam}/versioning/removed/api-version:${version}`;
  const userAgentInfo = `azsdk-js-versionning-removed-modular-rest/1.0.0-beta.1`;
