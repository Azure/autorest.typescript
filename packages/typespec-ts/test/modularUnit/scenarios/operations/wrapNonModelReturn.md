...
# wrap-non-model-return wraps ARM-style HEAD existence check as boolean body property

When both `wrap-non-model-return` and `head-as-boolean` are enabled, an ARM-style HEAD resource existence check
(`checkExistenceAtManagementGroupScope`) generates `CheckExistenceAtManagementGroupScopeResponse = { body: boolean }`.
This models the real Azure ARM pattern: `checkExistenceAtManagementGroupScope is Extension.CheckExistence<Extension.ManagementGroup<"groupId">, DeploymentExtended, Error = CloudError>`.

## TypeSpec

```tsp
@route("/providers/Microsoft.Management/managementGroups/{groupId}/providers/Microsoft.Resources/deployments/{deploymentName}")
@head
op checkExistenceAtManagementGroupScope(
  @path groupId: string,
...
```ts models alias CheckExistenceAtManagementGroupScopeResponse
export type CheckExistenceAtManagementGroupScopeResponse = { body: boolean };
```
...
```ts operations
import { TestingContext as Client } from "./index.js";
import { CheckExistenceAtManagementGroupScopeResponse } from "../models/models.js";
import { expandUrlTemplate } from "../static-helpers/urlTemplate.js";
import { CheckExistenceAtManagementGroupScopeOptionalParams } from "./options.js";
import {
  StreamableMethod,
  PathUncheckedResponse,
  createRestError,
...
export async function _checkExistenceAtManagementGroupScopeDeserialize(
  result: PathUncheckedResponse,
): Promise<CheckExistenceAtManagementGroupScopeResponse> {
  const expectedStatuses = ["204", "404"];
  if (!expectedStatuses.includes(result.status)) {
    throw createRestError(result);
  }
