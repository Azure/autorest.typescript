...
# discriminated union with @clientName
...
op create(@body skill: CustomNormalizer): {
  @body result: CustomNormalizer
};
```
...
/* eslint-disable @typescript-eslint/naming-convention */
/* eslint-disable @typescript-eslint/explicit-module-boundary-types */
/** model interface CustomNormalizer */
...
/** Type of TokenFilterName */
export type TokenFilterName = "arabic_normalization";
/** Type of CharFilterName */
...
import {
  CustomNormalizer,
  customNormalizerSerializer,
  customNormalizerDeserializer,
...
export function _createSend(
  ...
  return context
    ...
    .post({
      ...
      body: customNormalizerSerializer(skill),
    });
}

export async function _createDeserialize(
  result: PathUncheckedResponse,
): Promise<CustomNormalizer> {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    throw createRestError(result);
  }

  return customNormalizerDeserializer(result.body);
}
...
export async function create(
  ...
  const result = await _createSend(context, skill, options);
