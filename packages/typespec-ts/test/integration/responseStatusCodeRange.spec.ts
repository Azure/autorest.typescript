...
describe("Response Status Code Range RLC Client", () => {
  ...
  it("should handle error response with status code in range (494)", async () => {
    try {
      await client.path("/response/status-code-range/error-response-status-code-in-range").get();
  ...
  it("should handle error response with status code 404", async () => {
    try {
      await client.path("/response/status-code-range/error-response-status-code-404").get();
      assert.fail("Should have thrown an error");
    } catch (error: any) {
