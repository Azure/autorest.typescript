...
import { createParent, ParentContext, ParentClientOptionalParams } from "./api/index.js";
...
export class ParentClient {
  ...
  constructor(options: ParentClientOptionalParams = {}) {
    ...
    const userAgentPrefix = prefixFromOptions
      ...
      : `azsdk-js-client`;
    this._client = createParent({ ...options, userAgentOptions: { userAgentPrefix } });
    this.pipeline = this._client.pipeline;
    this._clientParams = { options };
  }

  getChildClient(blobName: string, options: ChildClientOptionalParams = {}): ChildClient {
    return new ChildClient(blobName, { ...this._clientParams.options, ...options });
  }
}
