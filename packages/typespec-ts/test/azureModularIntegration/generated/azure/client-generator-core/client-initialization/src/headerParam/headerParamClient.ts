...
export class HeaderParamClient {
  ...
  constructor(name: string, options: HeaderParamClientOptionalParams = {}) {
    ...
    const userAgentPrefix = prefixFromOptions
      ...
      : `azsdk-js-client`;
    this._client = createHeaderParam(name, { ...options, userAgentOptions: { userAgentPrefix } });
    this.pipeline = this._client.pipeline;
  }

  withBody(body: Input, options: WithBodyOptionalParams = { requestOptions: {} }): Promise<void> {
    return withBody(this._client, body, options);
  }

  withQuery(id: string, options: WithQueryOptionalParams = { requestOptions: {} }): Promise<void> {
