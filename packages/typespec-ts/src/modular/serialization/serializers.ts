...
export function serializeUnionInline(
  ...
  variants
    ...
    .forEach((v) => {
      throw Error(
        `TODO: Handle non-constant non-base variant of union ${type.name} with type name ${v.name} and kind ${v.kind}`
      );
    });
...
export function serializeEnumFunctionBody(
  ...
  const cases = type.values.map((value) => {
    const variant = type.isUnionAsEnum
      ? value.name
      : toPascalCase(`${type.name}. ${value.name}`);
    const serializedValue = serializeType({ ...options, type: value });
    const body =
      serializedValue === valueExpr ? undefined : `return ${serializedValue}`;
    return [
      value.valueType.encode === "string" ? `"${variant}"` : variant,
      body
    ] as const;
  });

  // Removes duplicate bodies and cases where the value is returned as-is
  const caseStatements = _(cases)
    .filter(([_, body]) => isDefined(body))
    .groupBy(([_, body]) => body)
    .values()
    .map(([variant, body]) => `case ${variant}: {${body}}`)
    .value();

  const switchStatement = caseStatements.length
    ? [`switch (${valueExpr}){`, ...caseStatements, "}"].join("\n")
    : undefined;

  return [switchStatement, `return ${valueExpr}`].filter(isDefined).join("\n");
}
