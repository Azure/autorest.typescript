...
export function buildSubpathIndexFile(project: Project, srcPath: string, subpath: string) {
  const apiFiles = project.getSourceFiles(`**/src/${subpath}/**`);
  const indexFile = project.createSourceFile(`${srcPath}/src/${subpath}/index.ts`);
