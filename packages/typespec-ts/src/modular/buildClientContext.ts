...
export function buildClientContext(
  ...
  // When experimental-subpath is enabled, add module-level API version management
  const experimentalSubpathEnabled =
    emitterOptions.modularOptions?.experimentalSubpath === true;
  const hasPreviewVersion =
    getLatestPreviewApiVersion(dpgContext) !== undefined;
  const shouldAddDynamicApiVersion =
    experimentalSubpathEnabled && hasPreviewVersion && apiVersionParam;
  ...
  // When experimental-subpath is enabled with dynamic API version, create context with a getter
  if (shouldAddDynamicApiVersion) {
  ...
  return context;`);
    ...
    // Original behavior when experimental-subpath is not enabled
