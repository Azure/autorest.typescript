...
import { buildExperimentalIndex } from "./buildExperimentalIndex.js";
...
export function buildExperimental(
  ...
  const experimentalRoot = [
    ...
    .join("/");
  const experimentalEmitterOptions: ModularEmitterOptions = {
    ...emitterOptions,
    modularOptions: {
      ...emitterOptions.modularOptions,
      sourceRoot: experimentalRoot
    }
  };
  const experimentalClientMap: [string[], SdkClientType<SdkServiceOperation>] =
    [[], clientMap[1]];
  const previewOperationGroups = previewInfo.previewMethods;
  if (previewOperationGroups.size > 0) {
    buildPreviewOperationFiles(
      dpgContext,
      experimentalClientMap,
      experimentalEmitterOptions,
    ...
    ensureApiBarrelFile(experimentalRoot);
    buildSubpathIndexFile(
      experimentalEmitterOptions,
      "classic",
      experimentalClientMap,
    ...
    project.createSourceFile(`${experimentalRoot}/api/index.ts`, "", {
      overwrite: true
    });
  }
...
/**
 * Ensure the root experimental/api/index.ts barrel file exists.
 ...
 */
function ensureApiBarrelFile(experimentalRoot: string): void {
  const project = useContext("outputProject");
  const barrelPath = `${experimentalRoot}/api/index.ts`;
