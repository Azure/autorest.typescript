...
function emitBodyParameter(
  ...
  if (body.bodyKind === "single") {
    ...
    // TODO: support multipart body parameters
    // https://github.com/Azure/autorest.typescript/issues/2422
    return undefined;
  }
  return undefined;
}
...
function emitBasicOperation(
  ...
  // any would skip type checking and setting the type to `any | undefined` doesn't make sense
  ...
  if (
    ...
    // skip body parameter if it is not a body parameter
    if (!bodyParameter) {
      bodyParameter = undefined;
    } else if (
