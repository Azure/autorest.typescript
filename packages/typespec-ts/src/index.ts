  ...
  async function generateMetadataAndTest(context: SdkContext) {
    ...
    if (shouldGenerateMetadata) {
      ...
      if (option.isModularLibrary) {
        ...
        // When this is an Azure monorepo modular package, check whether the existing
        // package.json contains AutoRest-specific (Swagger-generated) dependencies.
        // AutoRest uses @azure/core-client, @azure/core-tracing, @azure/core-paging,
        // and @azure/core-lro ^2.x — none of which are used by the TypeSpec modular emitter.
        // If any are detected, the full dependencies and devDependencies are replaced
        // with the canonical TypeSpec values from getAzureMonorepoDependencies.
        if (option.azureSdkForJs) {
          let existingPkgJson: Record<string, any> = {};
          try {
            existingPkgJson = await fsextra.readJson(existingPackageFilePath);
          } catch (_e) {
            // If the file can't be read, skip migration detection
          }
          ...
          if (hasAutorestDeps) {
            ...
            const depConfig: AzureMonorepoInfoConfig = {
              name:
                rlcClient.options?.packageDetails?.name ??
                rlcClient.libraryName,
              version:
                rlcClient.options?.packageDetails?.version ?? "1.0.0-beta.1",
              description:
                rlcClient.options?.packageDetails?.description ??
                `A generated SDK for ${rlcClient.libraryName}.`,
              moduleKind: option.moduleKind ?? "esm",
              withTests: option.generateTest === true,
              withSamples: option.generateSample === true,
              hasLro: hasPollingOperations(rlcClient),
              specSource: "TypeSpec",
              clientFilePaths: [],
            ...
            } = getAzureMonorepoDependencies(depConfig);
            modularPackageInfo = {
              ...modularPackageInfo,
              dependencies: canonicalDeps,
              devDependencies: canonicalDevDeps
