...
export function buildSnippets(model: RLCModel, clientName?: string) {
    ...
    if ((model?.options?.packageDetails?.scopeName === "azure" ||
        model?.options?.packageDetails?.scopeName === "azure-rest") && model.options.addCredentials && model.options.azureSdkForJs) {
        return {
