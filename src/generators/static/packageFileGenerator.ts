...
function restLevelPackage(packageDetails: PackageDetails) {
  ...
  if (generateTest) {
    ...
    if (azureSdkForJs) {
      ...
      packageInfo.scripts["unit-test:node"] = "dev-tool run test:node-ts-input -- --timeout 1200000 --exclude 'test/**/browser/*.spec.ts' 'test/**/*.spec.ts'";
      packageInfo.scripts["integration-test:node"] = "dev-tool run test:node-js-input -- --timeout 5000000 'dist-esm/test/**/*.spec.js'";
    } else {
