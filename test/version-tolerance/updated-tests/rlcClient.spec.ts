...
import DPG, { DPGClient } from "../generated/rlc-updated/src";
...
describe(`RLC Version Tolerance ${phase} phase`, async () => {
  ...
  describe("path", () => {
    ...
    it("Body payload required to optional", async () => {
      ...
      const result2 = await client.path("/serviceDriven/parameters").post({
    ...
    it("Add a new delete operation", async () => {
      const result = await client.path("/serviceDriven/parameters").delete();

      assert.equal(result.status, "204");
    });

    it("Add a new operation path", async () => {
      const result = await client.path("/serviceDriven/newPath").get();
