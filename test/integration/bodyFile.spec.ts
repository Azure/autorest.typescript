...
import { readStreamToBuffer, countBytesFromStream } from "../utils/stream-helpers";
  ...
  if (isNode) {
    describe("node.js", () => {
      it("getFile supports readableStream", async () => {
        ...
        const bufferedResult = await readStreamToBuffer(result.readableStreamBody!);

        const expectedBufferedResult = await fs.readFile(joinPath(
          __dirname, "..", "..", "node_modules", "@microsoft.azure", "autorest.testserver", "routes", "sample.png"
        ));

        assert.deepEqual(bufferedResult, expectedBufferedResult, "File returned by getFile does not match expected file.");
      });
