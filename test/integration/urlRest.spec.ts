...
describe("Integration tests for UrlRest with ShortcutMethods", () => {
  let client: UrlRestClientRestClient;
  ...
  describe("paths", () => {
    ...
    it("should work when path has  multi-byte byte values", async () => {
      //TODO: Check browser compatibility
      const byteArrayString = "5ZWK6b2E5LiC54ub54uc76ex76Ss76ex76iM76ip";
      const result = await client.paths.byteMultiByte(byteArrayString);
    ...
    it("should work when path has string", async () => {
      const result = await client.paths.stringEmpty("");
      assert.strictEqual(result.status, "200");
    ...
    it("should work when path has string unicode", async () => {
      const result = await client.paths.stringUnicode("啊齄丂狛狜隣郎隣兀﨩");
      assert.strictEqual(result.status, "200");
    });
    ...
    it("should work when path has base64url encoded string", async () => {
      const result = await client.paths.base64Url("bG9yZW0");
      assert.strictEqual(result.status, "200");
    });

    it("should work when path has a paramaeter in UnixTime format", async () => {
      const result = await client.paths.unixTimeUrl("1460505600");
      assert.strictEqual(result.status, "200");
    });

    it("should work when path has datetime", async () => {
      const result = await client.paths.dateTimeValid("2012-01-01T01:01:01Z");
      assert.strictEqual(result.status, "200");

      await client.paths.dateTimeNull(null as any);
    });

    it("should work when path has date", async function() {
      const result = await client.paths.dateValid("2012-01-01");
      assert.strictEqual(result.status, "200");
      assert.ok("called dateValid successfully");
    });
    ...
    it("should work when path has enum", async function() {
      try {
        await client.paths.enumValid("" as any);
      } catch (error) {
        assert.equal(
          error.message,
          ` is not a valid value for enumPath. The valid values are: ["red color","green color","blue color"].`
      ...
      try {
        await client.paths.enumValid(null as any);
      } catch (error) {
        assert.equal(error.message, `enumPath cannot be null or undefined.`);
      }

      const result = await client.paths.enumValid("green color");
      assert.strictEqual(result.status, "200");
    });

    it("should work when path has bool", async function() {
      const result = await client.paths.getBooleanTrue(true);
      const result1 = await client.paths.getBooleanFalse(false);
      assert.strictEqual(result.status, "200");
      assert.strictEqual(result1.status, "200");
      assert.ok("Both calls succeeded");
    });
    ...
    it("should work when path has double decimal values", async function() {
      const result = await client.paths.doubleDecimalNegative(-9999999.999);

      const result1 = await client.paths.doubleDecimalPositive(9999999.999);
      assert.strictEqual(result.status, "200");
      assert.strictEqual(result1.status, "200");
      assert.ok("Both calls succeeded");
    });
