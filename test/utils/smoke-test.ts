...
const smokePath = joinPath(".", "test", "smoke", "generated");
...
const generateFromReadme = async (readmeUrl: string) => {
  ...
  const output = joinPath(smokePath, projectName);
...
const verifyLibrary = async (readmeUrl: string): Promise<SmokeResult> => {
  ...
  return { readme: readmeUrl, success, error };
};
...
const verifyLibraries = async (readmes: string[]) => {
  ...
  logSuccess(
    `Finished smoke testing ${readmes.length}/${results.length} swaggers validates`
  );
};
...
verifyLibraries(chunk).catch(error => {
  logError(error);
  process.exit(-1);
});
