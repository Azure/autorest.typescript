...
//appServiceEnvironments.getInboundNetworkDependenciesEndpoints
async function getInboundNetworkDependenciesEndpoints() {
  const resourceGroupName = "Sample-WestUSResourceGroup";
  const name = "SampleAse";
  const resArray = new Array();
  for await (let item of client.appServiceEnvironments.getInboundNetworkDependenciesEndpoints(
    resourceGroupName,
    name
  )) {
    resArray.push(item);
